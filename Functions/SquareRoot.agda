...
module Functions.SquareRoot where
...
open import Functions.Exp using (exp)
open import Implementations.Nat using (natLog2Floor)
open import Implementations.Int using (natAbs; intRem)
...
import Implementations.Nat (suc)
import Implementations.Int (pos, negsuc)
...
sqrtQStep : ∀ {a : Set} {{ara : AppRationals a}}
                 -> Nat -> (x b : a) -> a
...
smallSqrtQ : ∀ {a : Set} {{ara : AppRationals a}}
...
-- whichever is even
sqrtQ : ∀ {a : Set} {{ara : AppRationals a}}
           -> Σ0 a (λ x -> null ≤ x) -> C a
...
sqrtQUc : ∀ {a : Set} {{ara : AppRationals a}}
...
-- A correctness proof:
{-
@0 sqrtIsCorrect : ∀ {a : Set} {{ara : AppRationals a}}
...
{-
@0 sqrtIsExpHalf : ∀ {a : Set} {{ara : AppRationals a}}
